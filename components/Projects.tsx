'use client'
import Section from './Section'
import { motion } from 'framer-motion'

const projects = [
  {
    title: '项目案例 A（占位）',
    type: 'C 端产品',
    summary: '社群类产品体验设计（占位）',
    roles: ['信息架构设计', '用户路径优化', '交互策略'],
    result: ['关键指标提升（占位）', '转化率优化（占位）'],
    tags: ['增长', '社群', '小程序']
  },
  {
    title: '项目案例 B（占位）',
    type: 'B 端企业系统',
    summary: '企业级售后维修流程线上化（占位）',
    roles: ['用户调研', '业务建模', '用户旅程', '系统架构设计'],
    result: ['流程时长降低（占位）', '处理效率提升（占位）'],
    tags: ['流程', '可视化', '数据联通', '结构化']
  },
  {
    title: '项目案例 C（占位）',
    type: '企业 SaaS',
    summary: '复杂流程优化与一致性设计（占位）',
    roles: ['用户旅程分析', '异常流程设计', '数据驱动优化'],
    result: ['流程清晰化（占位）', '机制完善（占位）', '一致性提升（占位）'],
    tags: ['SaaS', '一致性', '数据驱动']
  }
]

export default function Projects() {
  return (
    <Section id="projects" title="Projects">
      <div className="grid md:grid-cols-3 gap-6">
        {projects.map((p) => (
          <motion.div
            key={p.title}
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.2 }}
            transition={{ duration: 0.4 }}
            whileHover={{ y: -6, scale: 1.01, transition: { type: 'spring', stiffness: 260, damping: 18 } }}
            className="group card rounded-xl border border-neutral-100 bg-white p-6 shadow-card"
          >
            <div className="flex items-center justify-between">
              <div className="text-lg font-semibold">{p.title}</div>
              <span className="chip text-xs px-2 py-1 rounded bg-neutral-100">{p.type}</span>
            </div>
            <p className="mt-3 text-sm text-neutral-700">{p.summary}</p>
            <div className="mt-4">
              <div className="text-sm font-medium">职责</div>
              <ul className="mt-2 grid grid-cols-2 gap-2 text-sm text-neutral-700">
                {p.roles.map((r) => (
                  <li key={r} className="leading-relaxed">{r}</li>
                ))}
              </ul>
            </div>
            <div className="mt-4">
              <div className="text-sm font-medium">成果</div>
              <ul className="mt-2 space-y-1 text-sm text-neutral-700">
                {p.result.map((r) => (
                  <li key={r}>{r}</li>
                ))}
              </ul>
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              {p.tags.map((t) => (
                <span
                  key={t}
                  className="chip text-xs px-2 py-1 rounded-full bg-neutral-100 group-hover:bg-primary/10 transition-colors"
                >
                  {t}
                </span>
              ))}
            </div>
          </motion.div>
        ))}
      </div>
    </Section>
  )
}
